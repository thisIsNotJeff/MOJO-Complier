...
public abstract class Exp {
    ...
    private static CONST CONST(BigInteger v) {
        return new CONST(v);
    }
    ...
    public static class Ex extends Exp {
        ...
        public Tree.Stm unCx(Label t, Label f) {
            ...
            if (exp instanceof CONST) {
                ...
                // reaches exit
                Label[] targets =
                    (c.value.signum() == 0) ? new Label[] { f, t }
                        : new Label[] { t, f };
                return JUMP(NAME(targets[0]), targets);
            }
            return BNE(exp, CONST(BigInteger.ZERO), t, f);
        }
    ...
    public static class Nx extends Exp {
        ...
        public Tree.Stm unCx(Label t, Label f) {
            if (stm instanceof MOVE)
                return BNE(unEx(), CONST(BigInteger.ZERO), t, f);
            return null;
        }
    }
    ...
    public abstract static class Cx extends Exp {
        ...
        public Tree.Exp unEx() {
            ...
            return ESEQ(SEQ(SEQ(MOVE(TEMP(r), CONST(BigInteger.ONE)), unCx(t, f)),
                            SEQ(SEQ(LABEL(f), MOVE(TEMP(r), CONST(BigInteger.ZERO))),
                                LABEL(t))), TEMP(r));
        }
        ...
        public static class IfThenElseExp extends Cx {
            ...
            public Tree.Exp unEx() {
                ...
                if (cExp instanceof CONST) {
                    BigInteger c = ((CONST) cExp).value;
                    return (c.signum() != 0) ? tExp.unEx() : fExp.unEx();
                }
                ...
                if (tExp instanceof CONST && fExp instanceof CONST) {
                    BigInteger t = ((CONST) tExp).value;
                    BigInteger f = ((CONST) fExp).value;
                    if (t.signum() != 0 && f.signum() == 0)
                        return cond.unEx();
                    if (t.signum() == 0 && f.signum() != 0)
                        return XOR(cond.unEx(), CONST(BigInteger.ONE));
                    if (t.equals(f)) {
                        Label j = new Label();
            ...
            public Tree.Stm unNx() {
                ...
                if (cExp instanceof CONST) {
                    BigInteger c = ((CONST) cExp).value;
                    return (c.signum() != 0) ? tExp.unNx() : fExp.unNx();
                }
            ...
            public Tree.Stm unCx(Label tt, Label ff) {
                ...
                if (cExp instanceof CONST) {
                    BigInteger c = ((CONST) cExp).value;
                    return c.signum() != 0 ? tExp.unCx(tt, ff) : fExp.unCx(tt, ff);
                }
                Tree.Exp tExp = this.tExp.unEx();
                Tree.Exp fExp = this.fExp.unEx();
                Tree.Stm tStm, fStm;
